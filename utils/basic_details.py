...
def extract_resume_info_using_llm(resume_content):
    ...
    response_str = get_response_from_llm(final_prompt)
    response = parse_json_response(response_str)

    if response is None:
        # Log or print the raw LLM response for debugging
        print("LLM response parsing failed. Raw response was:")
        print(response_str)
        return None, None  # or fallback values

    name = response.get("name", None)
    resume_highlights = response.get("resume_highlights", None)
